...
def main(argv=None) -> int:
    ...
    print()
    print("And they are NOT measured on the same basis, which matters more")
    print("than the noise does. The general-purpose tools are handed the raw")
    print("CSV bytes, because that is what compressing a CSV actually is.")
    print("Parquet, ORC, Feather and Polypress are handed an already-parsed")
    print("table, so none of them is charged for reading the CSV -- the cost")
    print("every one of them would really pay. Read the columnar rows and the")
    print("polypress row against each other; read either against gzip only as")
    print("a rough guide.")
